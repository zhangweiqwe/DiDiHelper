...
static void PtraceSetRegs(pid_t pid, struct pt_regs *regs) {
    ...
    long l = ptrace(PTRACE_SETREGSET, pid, NT_PRSTATUS, &regsvec);

    if (DEBUG) {
        printf("PtraceSetRegs%ld %d\n", l, pid);
    }
#else
...
long CallRemoteFunctionFromNamespace(pid_t pid, long function_addr, long return_addr, long *args,
                                     size_t argc) {

    if (DEBUG) {
        printf("------>CallRemoteFunctionFromNamespace %d<-1\n", pid);
    }
#if defined(__aarch64__)
...
#define REGS_ARG_NUM    4
    ...
    if (DEBUG) {
        printf("------>CallRemoteFunctionFromNamespace %d %d<\n", pid, REGS_ARG_NUM);
    }
    ...
    if (argc > REGS_ARG_NUM) {
        ...
        if (DEBUG) {
            printf("------>CallRemoteFunctionFromNamespace %s<0\n", "argc > REGS_ARG_NUM");
        }
        regs.ARM_sp -= (argc - REGS_ARG_NUM) * sizeof(long);
        long *data = args + REGS_ARG_NUM;
        PtraceWrite(pid, (uint8_t *) regs.ARM_sp, (uint8_t *) data,
