...
long GetModuleBaseAddr(pid_t pid, const char *module_name) {
    ...
    if (DEBUG) {
        printf("=== GetModuleBaseAddr begin%d %s\n", pid, module_name);
    }
    ...
    if (DEBUG) {
        printf("=== GetModuleBaseAddr end%ld \n", base_addr_long);
    }
...
long GetRemoteFuctionAddr(pid_t remote_pid, const char *module_name, long local_function_addr) {
    ...
    if (DEBUG) {
        ...
        printf("GetRemoteFuctionAddr remote_pid%d pid%d  remote_base_addr%ld local_base_addr%ld\n",
               remote_pid, pid, remote_base_addr,
               local_base_addr);
    }
