...
void GetElfHeader(Elf64_Ehdr* elf_header, FILE* elf_file) {
...
size_t GetShstrtabContent(char** shstrtab_content, FILE* elf_file) {
    ...
    off_t shstrtab_header_offset = elf_header->e_shoff + elf_header->e_shstrndx * sizeof(Elf64_Shdr);
    if (DEBUG) {
        printf("shstrtab header offset: %lx\n", shstrtab_header_offset);
    }
    free(elf_header);
    Elf64_Shdr* shstrtab_header = (Elf64_Shdr*) malloc(sizeof(Elf64_Shdr));
    fseek(elf_file, shstrtab_header_offset, SEEK_SET);
    fread(shstrtab_header, sizeof(Elf64_Shdr), 1, elf_file);
...
void GetSectionHeaderByName(Elf64_Shdr* section_header, FILE* elf_file, const char* target_section_name) {
    ...
    for(int i = 0; i < section_count; ++i) {
        ...
        fread(section_header, sizeof(Elf64_Shdr), 1, elf_file);
        ...
        base_section_header_offset += sizeof(Elf64_Shdr);
    }
...
void PatchRemoteGot(pid_t pid, const char* library_path, long original_function_addr, long target_function_addr) {
    ...
    Elf64_Shdr* got_section_header = (Elf64_Shdr*) malloc(sizeof(Elf64_Shdr));
